...
static void UART_SendString(const u8 * const string, void (* const UART_SendByte)(const u8 data)){
    u8 i = 0;
...
static void UART_Send9BitString(const u16 * const string, void (* const UART_Send9BitData)(const u16 data)){
    u8 i = 0;
...
static const u8 * volatile asynchronousString_Send = NULL;
static void (* volatile asynchronous_UART_SendByte_NoBlock)(const u8 data);
...
static void ISR_UART_SendString_Asynchronous(void){
    ...
    /* Send string until null */
    if(asynchronousString_Send[i] != '\0') {
        ...
        asynchronous_UART_SendByte_NoBlock('\0');
        i = 1;
    }
}

static void UART_SendString_Asynchronous(const u8 * const string, void (* const UART_SendByte_NoBlock)(const u8 data), void (* const UART_TX_InterruptEnable)( void (* const ptrCallback)(void) ) ){
...
static void UART_Send9BitString_Asynchronous(const u8 * const string, void (* const UART_Send9BitData_NoBlock)(const u16 data)){
    u8 i = 0;
    ...
    /* Send null byte */
    if(UART_Send9BitData_NoBlock == UART0_Send9BitData_NoBlock) {
        UART0_Send9BitData('\0');
    } else if(UART_Send9BitData_NoBlock == UART1_Send9BitData_NoBlock) {
        UART1_Send9BitData('\0');
    }
}

/*-- Send String (Synchronous) with Checksum protection --*/
static void UART_SendString_Checksum(const u8 * const string, void (* const UART_SendByte)(const u8 data)) {
    u8 i = 0;
    u16 checksum = 0;
    ...
    UART_SendByte( (u8)(checksum >> 8) );
    UART_SendByte( (u8)checksum );
}

/*--------- Send Integer (Synchronous) ------------*/
static void UART_SendInteger(s32 integer, void (* const UART_SendByte)(const u8 data)) {
    u8 i = 0, remainder, string[10];
...
static void UART_SendFloat(float number, const u8 precision, void (* const UART_SendByte)(const u8 data)) {
    f32 fraction;
    u32 integer;
    u8 i = 0;
    ...
    integer = (u8)number;               /*!< Get integer part */
    ...
    fraction = (u8)fraction;     /*!< Remove the fraction integer part */

    UART_SendByte(integer);             /*!< Send integer part */
    UART_SendByte('.');                 /*!< Send decimal point */   
    UART_SendByte((u8)fraction);        /*!< Send fractional part */
}

/*--------- Receive String (Synchronous) ------------*/
static ERROR_STATUS_t UART_ReceiveString(u8 * const string, ERROR_STATUS_t (* const UART_ReceiveByte)(u8 * const data)) {
    ERROR_STATUS_t errorStatus = ERROR_NO;
    u8 i = 0;

    /* Receive string */
    errorStatus = UART_ReceiveByte(&string[i]);
    while(string[i] != '#') {
...
static ERROR_STATUS_t UART_Receive9BitString(u16 * const string, ERROR_STATUS_t (* const UART_Receive9BitData)(u16 * const data)) {
    ERROR_STATUS_t errorStatus = ERROR_NO;
    u8 i = 0;
    ...
    while(string[i] != '#') {
        ...
        ++i;
        errorStatus = UART_Receive9BitData(&string[i]);
    }
...
static u8 * asynchronousString_Receive = NULL;
static ERROR_STATUS_t (*asynchronous_UART_ReceiveByte_NoBlock)(u8 * const data);
...
static void UART_ReceiveString_Asynchronous_Callback(void){
    ...
    if(asynchronousString_Receive[i] != '\0') {
        ++i;
    }else {
        /* Disabling interrupt before sending null byte */
...
/* TODO: Not tested */
static void UART_ReceiveString_Asynchronous(const u8 * const string, ERROR_STATUS_t (* const UART_ReceiveByte_NoBlock)(u8 * const data), void (* const UART_RX_InterruptEnable)( void (* const ptrCallback)(void) ) ){
    ...
    asynchronousString_Receive = (u8 *)string;
...
static ERROR_STATUS_t UART_ReceiveString_Checksum(u8 * const string, ERROR_STATUS_t (* const UART_ReceiveByte)(u8 * const data)) {
    ERROR_STATUS_t errorStatus = ERROR_NO;
    u16 checksumCalculated = 0;
    u16 checksumReceived = 0;
    u8  size = 0;
    ...
    UART_ReceiveByte( (u8 *)&checksumReceived + 1 );
    UART_ReceiveByte( (u8 *)&checksumReceived );
